  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        exec(`cat ${rootfs}/etc/*-release; cat ${rootfs}/usr/lib/os-release; cat ${rootfs}/etc/openwrt_release`, function (error, stdout) {
          //if (!error) {
          /**
           * @namespace
...
function uuid(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        exec(`( cat ${rootfs}/var/lib/dbus/machine-id ${rootfs}/etc/machine-id 2> /dev/null || hostname ) | head -n 1 || :`, function (error, stdout) {
          if (!error) {
